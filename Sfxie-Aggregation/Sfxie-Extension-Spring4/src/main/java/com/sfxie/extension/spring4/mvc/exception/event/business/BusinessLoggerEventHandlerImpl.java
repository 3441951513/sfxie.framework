package com.sfxie.extension.spring4.mvc.exception.event.business;
...
public class BusinessLoggerEventHandlerImpl extends LoggerEventHandler{
	...
	public void handleEvent(LoggerEvent event) {
    	...
    	if(null!=ExceptionContainer.getXmlParameters() && ExceptionContainer.getXmlParameters().length>0){
    		for(int i=0;i<ExceptionContainer.getXmlParameters().length;i++){
    			Object obj = ExceptionContainer.getXmlParameters()[i];
    			try {
    				sb.append("\r\n    参数(xml) ");
					sb.append(i);
					sb.append(": \r\n\t");
