...
import com.sfxie.extension.redis.redisson.proxy.RedissonProxy;
import com.sfxie.extension.redis.redisson.transaction.IRedissonTransactionOperator;

public class MyRedisson  implements RedissonClient {

    private final CommandExecutor commandExecutor;
    private final ConnectionManager connectionManager;
    private final Config config;
    
//    private RedissonProxy redissonProxy;

    private final UUID id = UUID.randomUUID();

    MyRedisson(Config config) {
        this.config = config;
    ...
    private RedissonProxy getRedissonProxy(){
    	return new RedissonProxy();
    }
    /**
     ...
     */
    public static MyRedisson create() {
...
//        config.useClusterServers().addNodeAddress("127.0.0.1:7000");
     ...
     * @return Redisson instance
     */
    public static MyRedisson create(Config config) {
    ...
    public <V> RList<V> getList(String name) {
		MyRedissonList<V> redissonList = new MyRedissonList<V>(commandExecutor, name);;
        return (MyRedissonList<V>) getRedissonProxy().getInstance(redissonList,new Class[]{CommandExecutor.class,String.class},new Object[]{commandExecutor,name});
	}
    /**
     * Returns distributed list instance by name.
     *
     ...
     * @return distributed list
     */
    public <V> RList<V> getTransactionList(String name,IRedissonTransactionOperator redissonTransactionOperator) {
    	MyRedissonList<V> redissonList = (MyRedissonList<V>) getList(name);
    	redissonList.setRedissonTransactionOperator(redissonTransactionOperator);
		return redissonList;
    }
    ...
    public <K, V> RMap<K, V> getMap(String name) {
    	RedissonMap<K, V> redissonMap = new RedissonMap<K, V>(commandExecutor, name);;
        return (RedissonMap<K, V>) getRedissonProxy().getInstance(redissonMap,new Class[]{CommandExecutor.class,String.class},new Object[]{commandExecutor,name});
    }
    ...
    public static void main(String[] args) {
    	...
    	addressList.add("192.168.10.74:7000");
    	Config config = new Config();
		...
		MyRedisson myRedisson = MyRedisson.create(config);
		RList<String> MyRedissonList = myRedisson.getList("testList");
		List<String> list  = new ArrayList<String>();
		Collections.copy(list, MyRedissonList);
		String string = MyRedissonList.get(0);
		string = "111";
		MyRedissonList.set(0, string);
		myRedisson.shutdown();
	}
}
