(function(win){
	...
	function _$dataGridReload(config){
		...
		if($('#'+dataGridId).datagrid('getPager').length>0){
			$('#'+dataGridId).datagrid('getPager').pagination('select');
		}else{
			$('#'+dataGridId).datagrid('reload',null);
		}
	}
	/**
...
//		var title = config.title;
		...
		var tipColor = config.color || 'red';
		_config.msg = _$getMsgString(_config.icon,'<span style="color:'+tipColor+';">'+_config.msg+'</span>');
	...
	function _$formSubmit(config){
		...
		var actType = config.actType || 'save';
		$('#'+formId).form('submit',{
		    ...
		    onSubmit: function(param){
		    	var isValidate;
		        param['actModel.actType'] = actType;
		        if(onSubmit){
	...
	function _$formLoadData(config){
		...
		parameter['actModel.actType'] = 'form';
		GoLive.ajax({
			url : url,
			async : true,
			data : parameter,
			...
			success : function(data) {
				...
				var prefixData = new Object();
				for(var p in data){
					prefixData[prefix+p] = data[p];
				}
...
//		self._config = config;
		...
		function _$loadData() {
			...
			$.extend(_config,self.data('_datagridConfig'),true);
			...
			var pageNumber;
			var pageSize;
			if(dataGrid.datagrid('getPager').length>0){
				var paginationObj = dataGrid.datagrid('getPager').pagination('options','pageNumber');
...
//			var pageList = _config.pageList || [10,20,30,50,100];
		...
		function decoratedDataGridColumn(queryParamArray,decoratedColumns,_config){
			var params = {
					dataGridColumnDecoratorParamEntity : GoLive.JSON.toJSON(queryParamArray)
			};
	...
	$.extend($.fn.validatebox.defaults.rules, {
	    password: {
	        validator: function(value,param){
	        	if(isNoValidate(param)){
	        		return true;
	        	}
//	        	var reg=/^(?=[`~!@#\$%\^&*\(\)\-_=\+\\\|\[\]\{\}:;\"\',.<>\/\?\d]*[a-zA-Z]+)(?=[a-zA-Z`~!@#\$%\^&*\(\)\-_=\+\\\|\[\]\{\}:;\"\',.<>\/\?]*\d+)[`~!@#\$%\^&*\(\)\-_=\+\\\|\[\]\{\}:;\"\',.<>\/\?\w]{10,16}$/;
	        	var reg = /^[a-zA-Z0-9_]*$/;
	        	if(!reg.test(value)){
	        		GoLive.EasyUI.Message.show({
	        			timeout:3000,
	...
	function isNoValidate(param){
		if(param && param.length>0 && param[param.length-1]===true){
    		return true;
    	}
	}
	/////////////////////////////////////////////////////添加easyui表单验证规则///////////////////////////////////////////////////////////////
