...
(function($){
	//浏览器操作
	GoLive.Browser = new Object();
	GoLive.Browser.getType = _$getBrowserType ;
	function _$getBrowserType(){
...
//			var dataoptions = 'data-options="modal:true,buttons:[{text:\'Save\',handler:function(){...}},{text:\'Close\',handler:closeFunc}]"';
			var dialogWindow = '<div title="附件上传" class="easyui-dialog" style="overflow:hidden;padding:3px;width:900px;height:600px;"  id="fileUploadDialog"> '+
									'<iframe frameborder="no" border="0" marginwidth="0" marginheight="0" id="fileUploadModelWindowIframe"  scrolling="no"  width="100%" height="100%"></iframe>'+
								'</div>';
			$("body").append($(dialogWindow));
	        $("#fileUploadModelWindowIframe",$("body")).attr("src",_configs.url+'?type='+_configs.type); //设置IFRAME的SRC;
...
//			$('.select2-search__field').attr('placeholder','请选择......');
   		...
   		standardTypeSelect.on("select2:select", function (e) {
   			...
   			if(_configs.onSelect){
   				_configs.onSelect($("#"+_configs.selectId).val(),e.params.data);
   			}
   		});
   		standardTypeSelect.on("select2:unselect", function (e) {
   			var optionId = getSelect2OptionId(e,_configs);
   			$("#"+optionId,select).data("isSelected",false);
   			if(_configs.onUnselect){
   				_configs.onUnselect($("#"+_configs.selectId).val(),e.params.data);
   			}
   		}); 
	...
	GoLive.Select = new Object();
	GoLive.Select.init = _$selectInit;
	/**
	 * 请求第二个下拉框的数据
	 */
	function _$selectInit(secondTypeDef){
